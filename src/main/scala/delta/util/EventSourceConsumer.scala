...
abstract class EventSourceConsumer[ID, EVT](
  ...
  def process(eventSource: ES, maxTickSkew: Int)(implicit ec: ExecutionContext): Future[Subscription] = {
    ...
    maxTickProcessed match {
      case None =>
        eventSource.maxTick().flatMap { maxEventSourceTick =>
          start(eventSource, maxEventSourceTick)(selector(eventSource), maxTickSkew)
        }
      case Some(maxTickProcessed) =>
        eventSource.maxTick().flatMap {
          case Some(maxEventSourceTick) =>
            resume(eventSource, maxEventSourceTick)(selector(eventSource), maxTickProcessed, maxTickSkew)
          case None =>
            throw new IllegalStateException(s"Highest processed tick is $maxTickProcessed, but event source is empty: $eventSource")
        }
    }
  }
  ...
  private def resume(es: ES, maxEventSourceTickAtStart: Long)(
      selector: es.Selector, maxTickProcessed: Long, maxTickSkew: Int)(
      implicit ec: ExecutionContext): Future[Subscription] = {
    val batchProcessingDone: Future[Option[BatchResult]] = {
      val batchProc = StreamPromise(batchProcessor(es))
      es.querySince(maxTickProcessed - maxTickSkew, selector)(batchProc)
      batchProc.future.map(Option(_))(Threads.PiggyBack)
    }
    batchProcessingDone.flatMap { batchResult =>
      val realtimeProcessor = this.realtimeProcessor(es, batchResult)
