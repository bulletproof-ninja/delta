...
extends Codec[String, BsonValue] {
  ...
  def encode(json: String): BsonValue =
    new JsonBsonParser(json).parse()

  def decode(bson: BsonValue): String =
    bson.getBsonType match {
      case DOCUMENT =>
        val entries = bson.asDocument.entrySet().iterator.asScala.map { entry =>
          s"""${toJson(entry.getKey)}:${decode(entry.getValue)}"""
        }
        entries.mkString("{", ",", "}")
      case ARRAY =>
        bson.asArray.iterator.asScala.map(this.decode).mkString("[", ",", "]")
      case STRING =>
