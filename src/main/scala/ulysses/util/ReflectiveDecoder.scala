...
/**
  ...
  * arguments, 1. `Short`, 2. `SF` (the encoding format) and
...
abstract class ReflectiveDecoder[EVT: ClassTag, SF <: AnyRef: ClassTag] {
  ...
  private def decoderNotFound(evtName: String, noVersion: Boolean): Nothing = {
    val versionArg = if (noVersion) "" else "version: Short, "
  ...
  def decode(name: String, version: Short, data: SF): EVT = {
    ...
    decoderMethod.invoke(this, java.lang.Short.valueOf(version), data).asInstanceOf[EVT]
  }
