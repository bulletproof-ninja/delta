...
object LamportTicker {
  ...
  private[this] val subscribingClocks = {
      def factory(es: EventSource[_, _]): LamportTicker = {
    ...
    new Memoizer[EventSource[_, _], LamportTicker](factory)
  }

  /**
    * Internally managed Lamport ticker.
    */
  def apply(es: EventSource[_, _]): LamportTicker = subscribingClocks(es)
